class ContractorApplication < ApplicationRecord
  ...
  validate :address_line3_is_valid
  with_options if: :located_in_united_states? do |application|
    application.validate :business_identification_number_is_valid
    application.validate :subject_to_backup_withholding_is_selected
  end
  with_options if: :submitted_broadcaster_application? do |broadcaster_app|
    broadcaster_app.validate :has_uploaded_verification?
  end

  # If user has pending application, do not let them modify the record
  ...
  before_create :timestamp_data_consent
  ...
  def business_identification_number_is_valid
    ...
    if business_identification_number.include?('***')
      p "found obfuscated BIN, excluding from parameter hash"
      restore_attribute("business_identification_number")
    end
  end
